...
class UsersDisplay(DisplayAbstract):
    ...
    def init_components_logged(self) -> None:
        ...
        self.__window = sg.Window(
            "Logged Users Menu", layout, size=(data.HEIGHT, data.WIDTH), finalize=True
        )
    ...
    def get_data(self) -> tuple[str, str]:
        layout = [
            ...
            [
                sg.Text("Nickname:", size=(15, 1), font=data.FONT),
                sg.InputText(key="-NICKNAME-", font=data.FONT),
            ],
            [
                sg.Text("Password:", size=(15, 1), font=data.FONT),
                sg.InputText(key="-PASSWORD-", font=data.FONT, password_char="*"),
            ],
            [
                sg.Column(
                    [[sg.Button("Ok", size=(10, 1), font=data.FONT)]],
        ...
        self.__window = sg.Window(
            "Get User Data", layout, size=(data.HEIGHT, data.WIDTH), finalize=True
        )
        while True:
            event, values = self.__window.read()
            if event == "Ok":
    ...
    def show_user_data(self, nickname: str, id: UUID) -> None:
        layout = [
            ...
            [
                sg.Text(
                    f"Id: {id}", size=(50, 1), justification="center", font=data.FONT
                )
            ],
            [
                sg.Column(
        ...
        self.__window = sg.Window(
            "User Data", layout, size=(data.HEIGHT, data.WIDTH), finalize=True
        )
        while True:
            event, values = self.__window.read()
            if event == "Ok":
